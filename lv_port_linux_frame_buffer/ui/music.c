...
static lv_timer_t *music_slider_time; //计算进度条时间
static int voice = 100;
static int time_start = 0;//音乐的进度条
static int time_end = 0;
static pthread_t music_w_id;//写线程就只有开始有用（从写的demo copy过来就没改了）
static pthread_t music_r_id;
static int fd_fifo;					//创建有名管道，用于向mplayer发送命令
static int fd_pipe[2];				//创建无名管道,用于从mplayer读取命令
static pid_t mplayer_pid;           //mplayer的pid  用于kill
static int pre_btn_id;              //上一个btn手动更改btn——state
static int btn_music_id;
static int music_num;               //加载多少首，按键切换
static pthread_mutex_t mplayer_mutex;
static pthread_cond_t  mplayer_cond;
static int start; 
static int ready;     //线程唤醒丢失
static int btn_exchange;  //暂停播放按键切换    
#define MUSIC_PATH   "./"     
    ...
    if(code == LV_EVENT_CLICKED){
        ...
        if(obj == btn0){
            ...
            if(btn_exchange == 0){
                ...
                strcpy(cmd.cmd_name.name, "music control");
                strcpy(cmd.cmd_info.info, "resume\n");
                music_cmd_write(cmd_head, cmd);
...
{    
   ...
   // set_btn_state(cont_col,lv_obj_get_child(cont_col,btn_music_id));//自动加载第一首歌，在板子上突然不行了
...
{   
    send_cmd("pause\n");
    //send_cmd("volume 0 1\n");//在板子上mplayer这个命令不能调节并且直接暂停有噪声
    system("amixer sset 'DAC' 0");
    // send_cmd("loadlist ./music/music.lst\n");
	// while(start == 0);                        //自动加载第一首歌，在板子上突然不行了
	// send_cmd("get_time_length\n");
	//send_cmd("volume 100 1\n");
    // system("amixer sset 'DAC' 63");
	char buf[100];
	while(1)
	{
...
static void music_hand(void)
{
	...
	if(pid==0)								//子进程播放mplayer
	{
		...
		execlp("mplayer","mplayer","-ao","alsa","-slave","-quiet","-loop","0","-input","file=/tmp/my_fifo",MUSIC_PATH"music/稻香.mp3",NULL);
	}
	else
	{
        mplayer_pid = pid;
...
void music_cmd_handle(void)
{
    ...
    if(strcmp(cmd.cmd_name.name,"music create") == 0){
        ...
        //send_cmd("volume 0 1\n");
        system("amixer sset 'DAC' 0");
        sprintf(tmp, "loadfile %smusic/%s\n", MUSIC_PATH, name_buf);
        send_cmd(tmp);
        send_cmd("get_time_length\n");
        sprintf(tmp, "amixer sset 'DAC' %d", 63*voice/100);
        // send_cmd(tmp);
        system(tmp);
        pthread_mutex_lock(&lvgl_mutex);
       ...
       if(strcmp(cmd.cmd_info.info, "pause\n") == 0){
           system("amixer sset 'DAC' 0");
           send_cmd("pause\n");
       }else{
           char tmp[60];
        ...
        char tmp[60];
        int a = atoi(cmd.cmd_info.info);
        sprintf(tmp, "amixer sset 'DAC' %d", 63*a/100);
        system(tmp);
        //send_cmd(tmp);
